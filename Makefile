...
# Проверки безопасности разнесены по видам — SAST, SCA, секреты, — чтобы
# у каждого был свой прогон в CI и свой значок в README: заказчику нужен
# ответ «что показал разбор кода», а не «что-то из безопасности красное».
# `make security` зовёт все сразу.
.PHONY: security
security: sast sca secrets ## Проверки безопасности: статический разбор, зависимости, секреты в истории

# Инструменты ставятся каждый раз, а не «если их нет». Все берутся
...
.PHONY: sast
sast: ## SAST: статический разбор кода на Go (gosec)
...
.PHONY: sca
sca: ## SCA: известные уязвимости зависимостей сервера и клиента (govulncheck, npm audit)
	go install golang.org/x/vuln/cmd/govulncheck@latest
	$(SECURITY_TOOLS)/govulncheck ./...
	cd web && npm audit --audit-level=high

# Секреты — по всей истории, а не по рабочему дереву: ключ, убранный
# следующим коммитом, остаётся в истории и в каждом клоне. Ложные
# срабатывания названы в .gitleaksignore с причиной.
.PHONY: secrets
secrets: ## Секреты во всей истории git (gitleaks)
	go install github.com/zricethezav/gitleaks/v8@latest
	$(SECURITY_TOOLS)/gitleaks git . --no-banner --redact
...
tarball: build ## Собрать архив для установки из бинарника (dist/)
	...
	cp README.md README.ru.md CHANGELOG.md CHANGELOG.ru.md LICENSE NOTICE THIRD-PARTY.md "$(TARBALL_DIR)/"
...
bundle: ## Собрать комплект для установки без доступа в интернет (BUNDLE_ARCH=amd64|arm64)
	...
	cp README.md README.ru.md CHANGELOG.md CHANGELOG.ru.md LICENSE NOTICE THIRD-PARTY.md "$(BUNDLE_DIR)/"
	...
	@echo "  skopeo inspect docker-archive:takt-image.tar.gz --format '{{.Architecture}}'"
	@echo "  skopeo copy docker-archive:takt-image.tar.gz docker://<реестр>/takt:$(BUNDLE_VERSION)"
	@echo "  helm install takt takt-*.tgz --set image.repository=<реестр>/takt \\"
	@echo "    --set image.tag=$(BUNDLE_VERSION) --set baseURL=... --set database.existingSecret=takt-db"
