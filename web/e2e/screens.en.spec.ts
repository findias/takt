...
test('английские снимки для документации', async ({ page }) => {
  ...
  await page.getByRole('group', { name: /Move the supplier directory/ }).locator('.card-title').click()
  await page.getByRole('tab', { name: 'Work' }).click()
  await page.waitForTimeout(400)
  await page.screenshot({ path: `${SHOTS}/36-person-label.png` })
  await page.goto(boardUrl)

  await page.getByRole('group', { name: /Agree the delivery schedule/ }).locator('.card-title').click()
  ...
  await page.getByRole('group', { name: /Go through this week's requests/ }).locator('.card-title').click()
  await page.getByRole('tab', { name: 'Tasks' }).click()
  await page.getByText('SR-10492').waitFor()
  await page.screenshot({ path: `${SHOTS}/41-card-tickets.png` })
  await page.goto(boardUrl)

  await page.getByRole('group', { name: /Ship the warehouse release/ }).locator('.card-title').click()
  await page.getByRole('tab', { name: 'Work' }).click()
  await page.waitForTimeout(400)
