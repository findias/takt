...
test('шапка доски не съедает экран', async ({ page }) => {
  ...
  // 60% → 62% (26.09.2026): выпадающие списки стали высотой с поле
  // ввода, +2 пикселя на каждый из двух рядов инструментов. Замер — 431
  // на рабочей машине и 460 на машине проверок GitHub (шрифт шире,
  // название доски выше); порог — по худшему, 515 прежнего кода он
  // по-прежнему не пропускает.
  expect(top, 'первая карточка на телефоне').toBeLessThanOrEqual(760 * 0.62)
})
