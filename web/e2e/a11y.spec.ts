...
test('мельче шкалы на экране только монограмма аватара', async ({ page }) => {
  ...
  //
  // Кегль там не выбран, а выведен: буква обязана поместиться
  // в кружок. Поднять кегль, не подняв кружок, значит обрезать буквы;
  // поэтому проверка следит за обоими концами разом.
  //
  // Мерить обязательно самые широкие буквы, а не доставшиеся
  // демонстрационным данным, и обязательно в браузере, а не в уме.
  // Прежде в кружке стояли две заглавные, и выведены они были из
  // одного замера — «ШЩ» при кегле восемь занимали семнадцать
  // пикселей из двадцати (23.08.2026). На машине с другим системным
  // шрифтом те же «ШЩ» заняли двадцать один: продукт не везёт своего
  // начертания, и три пикселя запаса разброс системных шрифтов
  // не покрывают. Буква с тех пор одна.
  await page.setViewportSize({ width: 1440, height: 900 })
  await signInToDemo(page)
  await openDemoBoard(page)
  await expect(page.getByRole('region', { name: 'Очередь' })).toBeVisible()
