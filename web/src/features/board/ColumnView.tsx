...
type ColumnProps = {
  ...
  collapsed: boolean
  /** Доска разложена дорожками: колонка принадлежит дорожке, и пустой
   *  ей нельзя обещать то, чего перенос не сделает. */
  grouped: boolean
      ...
      >
        ...
        {props.cardIds.length === 0 &&
          ...
          (props.hiddenByFilter > 0 ? (
            ...
            <p className="empty">
              {props.grouped
                ? // Дорожка не принимает переноса: и перетаскивание, и меню
                  // переносят карточку в колонку, а дорожка выводится
