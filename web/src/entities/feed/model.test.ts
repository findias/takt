...
test('действие без установленной личности видно как таковое', () => {
  ...
  assert.equal(actorText(null), 'без подписи')
  assert.equal(actorText('Иван'), 'Иван')
})

// Снятие по сроку — своё событие, а не «unblocked» с пометкой: лента
// разбирает события по типу, и «вышел срок» — другое событие, чем
// «Борис снял блокировку». Автора у него нет, и строка читается как
// строка без автора, а не как запись с потерянным именем (ROADMAP 28.1).
test('блокировка, снятая по сроку, и смена срока читаются словами', () => {
  assert.equal(eventText(event('block_expired')), 'блокировка снята сама: вышел срок')
  assert.equal(actorText(null), 'без подписи')
  assert.match(
    eventText(event('block_until', { until: new Date(2026, 8, 24, 18, 0).toISOString() })),
    /^срок блокировки: до 24 сентября, 18:00$/,
  )
  assert.equal(eventText(event('block_until', { until: null })), 'блокировка стала бессрочной')
})
