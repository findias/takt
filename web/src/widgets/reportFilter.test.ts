...
import { downloadHref, parseReportFilter, presetPeriod, reportQuery } from './reportFilter.ts'

const NOW = new Date(2026, 8, 24, 21, 30)

test('без параметров — последние 90 дней по местному календарю', () => {
...
test('адрес разбирается и собирается обратно тем же', () => {
  ...
  assert.equal(reportQuery(f).toString(), raw)
})

test('испорченная ссылка даёт отчёт, а не отказ', () => {
  const f = parseReportFilter(new URLSearchParams('from=вчера&state=closed,done&priority=urgent'), NOW)
  assert.equal(f.from, '2026-06-27')
  assert.deepEqual(f.states, ['done'])
