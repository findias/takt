...
/**
 * Отказ, относящийся ко всему экрану: не загрузилась доска, не удалось
 * прочитать команду, отвергли webhook.
 *
 * Тот же живой регион, что и у отказа формы, и заведён по той же
 * причине — но с оформлением экрана, а не формы. До 23.08.2026 таких
 * мест было двадцать, и все двадцать были написаны одинаково:
 * `{error && <p className="error">{error}</p>}`. Условие тут и есть
 * поломка: узел появляется вместе с текстом, а диктор объявляет
 * изменение той области, которая уже была. Не видящему экрана отказ
 * доставался молчанием — то есть нажатая кнопка выглядела так, будто
 * ничего не произошло.
 */
