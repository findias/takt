...
import type { BoardAccess as Access, Member, Team, Visibility } from './api'

/**
 * Кому видна доска.
 *
 * Одно правило приходится объяснять прямо здесь, потому что оно неочевидно
 * и потому что база отказывает в нём отдельным ответом: закрыть доску можно
 * только вокруг себя. Иначе доска, закрытая вокруг чужих людей, стала бы
 * неисправимой — редактировать невидимую доску не может никто.
 */
