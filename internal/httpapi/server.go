...
func (s *Server) authed(next func(http.ResponseWriter, *http.Request, auth.Principal)) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if token, ok := bearer(r); ok {
			...
			// Ключ каталога сюда не ходит вовсе. Прежде за этим стояла
			// ещё и роль владельца организации, и сужение было главной
			// защитой: разрешения проверяются на восьми маршрутах
			// из девяноста семи, и на остальных ключ держала бы одна
			// роль, то есть ничто. Роль снята (миграция 0048), а сужение
			// осталось — оно и без неё уместно. Сужаем один ключ, а не
...
// команду нельзя.
//
// Отказ на занятый адрес называет вещи своими именами — «такая почта уже
// зарегистрирована, войдите», — и этим адреса становятся перечислимыми:
// кто угодно проверяет любой адрес одним запросом. Это принято сознательно,
// потому что скрыть занятость нечем: человеку, заводящему учётную запись,
// сказать надо, а подтверждения почты в системе нет — писать письма нечем.
