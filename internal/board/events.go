...
// EventPrefix — под этим именем виды событий уезжают подписчикам:
// «card.moved». Приставка одна на все, потому что событий не про карточку
// в потоке пока нет; появятся — у них будет своя.
const EventPrefix = "card."

// EventNames — имена событий для подписки, то есть виды с приставкой.
// Тем же списком отвечает сервер интерфейсу и описанию контракта: список,
// повторённый где-то ещё, разойдётся с этим в первый же день.
func EventNames() []string {
	out := make([]string, 0, len(EventKinds))
	for _, kind := range EventKinds {
		out = append(out, EventPrefix+kind)
	}
	return out
}
...
var EventKinds = []string{
	...
	"blocked",
	"block_until",
	// Причину поправили — интервал тот же, меняется только текст.
	"block_reason",
	"unblocked",
