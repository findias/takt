...
func ensureOtherOwnerExists(ctx context.Context, tx pgx.Tx, orgID, exceptUserID string) error {
	...
	// Считает база, и считает она людей. Ключ каталога владельцем больше
	// не рождается (миграция 0048), но поднять служебную личность
	// до владельца человеку никто не запрещает — а «другим владельцем»
	// она быть не должна: войти ею нельзя, и организация осталась бы
	// без управления (миграция 0047).
	var others int
	err = tx.QueryRow(ctx,
