...
var (
	...
	// Остальные разрешения такому ключу всё равно ничего не дадут:
	// за пределы /scim/v2 он не ходит. Отказ называет, что делать:
	// завести второй ключ.
...
const (
	...
	// Отдельное разрешение, потому что права у него свои: подразделения
	// и их состав открывает политика directory (миграция 0048), а не роль
	// владельца, которую ключ носил до неё. Людей RLS не касается вовсе.
...
// ключу открыт только /scim/v2. Прежде за этой проверкой стояла ещё
// и роль владельца, и держалась она одной ей: сорвись ключ с маршрута —
// и он владелец арендатора. Теперь за ней ничего не стоит, а права
// каталога кончаются в базе, на подразделениях и их составе.
...
func (s *Service) Create(ctx context.Context, orgID, actorID, name string, scopes []string, expiresAt *time.Time) (Client, error) {
	...
	// Ключ каталога остаётся наблюдателем, как и всякий ключ без права
	// писать доски. Роль владельца он носил ради двух таблиц —
	// подразделений и их состава, — а теперь их открывает собственное
	// право: политика directory и app_is_directory() (миграция 0048).
	// Роль была страховкой чужой системы: ключ лежит в Okta или Entra
	// и живёт годами, а в полосе /scim/v2 его держала проверка в коде,
	// а не политика — одного маршрута, обёрнутого s.authed вместо
	// s.scoped, хватило бы, чтобы он стал владельцем арендатора.
