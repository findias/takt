...
// где её требуют политики, и не выходит наружу, где она никому
// не обещалась.
...
func (s *Service) Create(ctx context.Context, orgID, actorID, name string, scopes []string, expiresAt *time.Time) (Client, error) {
	...
	// требуют именно этого. Всесильным ключ от этого не становится, но
	// удерживает его не роль, а вход: разрешения проверяются лишь
	// на четырёх маршрутах доски (расписать их по всем — это 4.3,
	// отложенный осознанно), и ключом каталога когда-то архивировалась
	// доска и выгружалась вся организация. Теперь такой ключ дальше
	// /scim/v2 не пускают вовсе.
	if seen[ScopeSCIM] {
		role = auth.RoleOwner
	}
