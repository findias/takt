...
var quiet = map[string]bool{
	...
	// Справка (help) держит оба языка сама, рядом: русские и английские
	// имена разделов и подписи оболочки стоят парой в одной строке.
	"help": true,
...
var quietFiles = map[string]bool{
	...
	// Словари импорта: так поле, приоритет и колонку называют чужие
	// выгрузки. Это образцы для сравнения, а не слова для человека.
	"importer/words.go": true,
	// То же для Jira: как приоритет называет её переведённый интерфейс.
	"importer/jira/words.go": true,
}
...
var logged = map[string]int{
	...
	// Ход выгрузки takt-fetch — в терминал тому, кто её запустил, как
	// и остальное у команд администратора.
	"progress": -1,
}
...
var formatted = map[string]int{
	...
	// Претензия к строке импорта: поле, значение, затем шаблон.
	"problem": 2,
}
