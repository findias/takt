...
type Card struct {
	...
	Parent string
	// ParentBoard — доска пакета, на которой лежит родитель, если не эта
	// (эпик на портфеле). Родитель находится среди переехавшего раньше;
	// не переехал — отчёт называет доску, которую перенести.
	ParentBoard string
	// ParentName — как родителя назвать человеку: номер и название.
	// Внешний ключ Jira — число, по которому эпик в Jira не найти.
	ParentName string
	Links      []Link
	Comments   []Comment
...
type Plan struct {
	...
	// Portfolio — доска источника — портфель эпиков: новая доска
	// заводится этого уровня.
	Portfolio bool
	// ForeignParts — части карточек этой доски, лежащие на других досках
	// пакета. Переехали раньше — связываются с родителем этим прогоном.
	ForeignParts []ForeignPart
}

// ForeignPart — связь «подзадача» между досками пакета: родитель на этой
// доске, часть — на другой. Ключи внешние.
