...
// Ловится именно номер версии — «v0.1.0-9-g003ba51-dirty» или
// «не задана», — а не любое слово «версия»: оно встречается
// и в тексте документов, и вырезать их целиком значило бы перестать
// сверять целые абзацы.
var подписьВерсии = regexp.MustCompile(`(?i)(верси[яи]|version)\s+(v[0-9][^\s<]*|не задана[^<]*)`)

func безВерсии(страница string) string {
	return подписьВерсии.ReplaceAllString(страница, "версия …")
}
