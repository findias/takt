...
**[Попробовать демо](https://takt-demo-latest.onrender.com)** — своя
организация с примерами досок на сутки, без регистрации. Стоит
на бесплатном тарифе и засыпает без посетителей, поэтому первый заход
...
![Канбан-доска: колонки размечены для потока](docs/ru/screenshots/доска.png)

**Интерфейс — на русском и английском.** Пока человек не выбрал язык
за своим именем в шапке, язык берётся из браузера; выбор хранится
в учётной записи и встречает его на любом устройстве. Английская
документация в [`docs/`](docs/overview.md) называет кнопки так, как
их показывает английский интерфейс, и снимки в ней английские. Та же
документация открывается справкой внутри приложения (`F1`).
...
[документация](docs/ru/индекс.md). **Что продукт обязуется не сломать**
...
назад: на пустом интерфейсе не видно ни вёрстки, ни метрик. Та же
организация по-английски, для английских снимков, — под
`anna@en.example.test` с тем же паролем.
...
[`docs/ru/установка.md`](docs/ru/установка.md).
...
- **Метки трёх областей.** Организации, подразделения или одной доски;
  всякий список говорит, откуда метка, а недостающую заводят прямо
  с карточки.
- **Блокировка с причиной и сроком.** Блокировка говорит, чего ждёт
  работа; со сроком она снимается сама, и время в блоке честно
  попадает в метрики.
...
- **Выгрузка для руководства.** Карточки за период по доскам,
  подразделениям, людям и меткам — одной плоской таблицей, со сводкой
  потока и графиками из тех же строк: Excel, CSV или JSON.
...
- **Переезд.** Из таблицы (CSV, Excel), из YouGile по его API или —
  в закрытый контур — из пакета переноса, который снаружи собирает
  отдельный выгрузчик [`takt-fetch`](docs/ru/выгрузчик.md) из YouGile,
  Jira, Kaiten или monday: подзадачи,
  обсуждения, история задач и выбор по каждому человеку.
...
в [`docs/ru/установка.md`](docs/ru/установка.md).
