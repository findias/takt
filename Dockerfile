...
# бывает нужен не alpine, а glibc-система или сертифицированная ОС,
# и второй Dockerfile под каждую разошёлся бы с этим так же быстро.
...
RUN set -eu; \
    ...
    # (Astra в том числе) они уже стоят. Ставить поверх — лишний слой
    # и лишний поход в сеть.
    ...
    # группа своя, разошёлся бы с ним молча. На debian и Astra
