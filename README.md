...
Документация для тех, кто пользуется: [обзор](docs/индекс.md),
[первые пятнадцать минут](docs/старт.md), [как сделать](docs/как.md),
[справочник](docs/справочник.md), [устройство](docs/устройство.md).
Этот файл — для того, кто ставит и держит.

Что продукт обязан делать, в каких пределах и чего он делать не будет —
в [`REQUIREMENTS.md`](REQUIREMENTS.md). Там же сказано, чем каждое
обещание закреплено и что мы ждём от того, кто ставит.
