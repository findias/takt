...
// Настройки приложения и чарт не должны расходиться молча.
//
// Приложение читает переменные окружения; чарт их передаёт. Ничем
// не было закреплено, что второе покрывает первое, — и расхождение
// обнаруживалось у заказчика: настройка есть в приложении, задать её
// через чарт нельзя, и остаётся `extraEnv`, то есть знание внутренних
// имён. Настройка, которую можно задать только зная внутреннее имя,
// это не настройка, а обходной путь.
//
// Сверяются два списка: что читает `config.Load` и что отрисовывает
// чарт. Исключения перечислены поимённо и с причиной — молчаливых
// исключений здесь быть не должно, иначе список снова разойдётся.
var задаётсяИначе = map[string]string{
	// Приезжает из образа: каталог статики лежит внутри него, и менять
	// его снаружи незачем — чужой путь означает пустой экран.
	"WEB_DIR": "задан в Dockerfile",
	// Тестовый стенд ветки — наш, на Render, а не у заказчика: в чарте
	// он был бы настройкой, которую ставящему нечем объяснить.
	"STAND": "только для нашего стенда на Render (deploy/stand/README.md)",
}

func TestChartCoversEverySettingTheAppReads(t *testing.T) {
	требуетHelm(t)
	...
	исходник, err := os.ReadFile(filepath.Join(корень, "internal", "config", "config.go"))
	if err != nil {
		t.Fatal(err)
	}
	читает := регулярка.FindAllStringSubmatch(string(исходник), -1)
	if len(читает) == 0 {
		t.Fatal("в конфигурации не нашлось ни одной переменной — проверка ничего не проверяет")
	}
	...
	for _, найдено := range читает {
		имя := найдено[1]
		if причина, известно := задаётсяИначе[имя]; известно {
			if strings.Contains(out, "- name: "+имя+"\n") {
				t.Errorf("%s объявлен исключением (%s), а чарт его передаёт: список исключений устарел",
					имя, причина)
			}
			continue
		}
		if !strings.Contains(out, "- name: "+имя) {
			t.Errorf("приложение читает %s, а чарт его не передаёт: "+
				"задать настройку можно будет только через extraEnv, то есть зная внутреннее имя",
				имя)
		}
	}
}
