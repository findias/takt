...
The first screen offers to create a team: on an empty installation the
first person to arrive creates the organisation and becomes its owner.
After that, registration is closed and that owner invites everyone else.
`SIGNUP` switches this: `open` lets anyone create an organisation (this
is how the demo stand runs), `closed` lets no one, which is what
installations with a corporate identity provider want.

Once installed, check that it was installed correctly:

```bash
docker compose run --rm app doctor     # or /app/takt doctor inside the pod
```

Readiness answers "the process is alive and the database responds";
`doctor` answers a different question: is this the right schema, the
right address — and therefore will a Secure cookie ever arrive — is the
identity provider reachable, and do the database notifications arrive
without which the board never refreshes itself.
...
YouGile, Jira (the cloud and your own installation), Kaiten (the cloud
and on-premises) and monday; for the last three see its page. For YouGile:
